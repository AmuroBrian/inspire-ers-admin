...
const AddModal = ({ isOpen, onClose, onAdd }) => {
  ...
  const fetchPlatformVersions = async (platform) => {
    try {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        if (data.platform === platform && typeof data.version === 'string') {
  ...
  // Compute the next version suggestions based on the highest version
  const getNextVersions = (platform, versions) => {
    ...
    const match = highest.match(/^v(\d+)(?:\.(\d+))?$/);
    if (!match) return ['v1'];
    const major = parseInt(match[1]);
    const minor = match[2] ? parseInt(match[2]) : 0;
    const suggestions = [];
    if (minor === 0) {
      for (let i = 1; i <= 9; i++) suggestions.push(`v${major}.${i}`);
      suggestions.push(`v${major + 1}`);
    } else {
      if (minor < 9) {
        for (let i = minor + 1; i <= 9; i++) suggestions.push(`v${major}.${i}`);
      }
      suggestions.push(`v${major + 1}`);
    }
  ...
  const handlePlatformChange = async (platform) => {
    ...
    if (platform) {
      const versions = await fetchPlatformVersions(platform);
      const nextVersions = getNextVersions(platform, versions);
      setFilteredVersions(nextVersions);
      setShowVersionSuggestions(true);
      setSelectedVersion(nextVersions[0] || 'v');
    }
  };
  ...
  // Prevent deleting the 'v' prefix in the version input
  const handleVersionChange = (e) => {
    let value = e.target.value;
    if (!value.startsWith('v')) value = 'v' + value.replace(/^v*/, '');
    if (value === '') value = 'v';
    setSelectedVersion(value);

    // Filter suggestions
    const filtered = filteredVersions.filter(version =>
      version.toLowerCase().startsWith(value.toLowerCase())
    );
